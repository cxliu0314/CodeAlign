...
def main():
    ...
    a = opt.model_dir.split('/')[-1]
    save_path = '/GPFS/data/changxingliu/HEAL/opencood/logs/dist_vis_'+a
    ...
    for i, batch_data in pbar:
        ...
        with torch.no_grad():
            ...
            if cav_content['agent_modality_list']==['m1', 'm2', 'm6']:
                ...
                feature_after = ouput_dict['feature_after']
                N, C, H, W = feature_before.shape
                codebook = model.multi_channel_compressor.Codebooks[0][0].cpu().numpy() # K, C
                feature_index = ouput_dict['feature_index']
                feature_index = torch.cat(feature_index, dim=0).float().squeeze(-1).reshape(-1, H, W) # N, H, W
                ...
                feature_after = warp_feature(feature_after, affine_matrix, record_len) # N, C, H, W
                feature_index = warp_feature(feature_index.unsqueeze(dim=1), affine_matrix, record_len, mode='nearest').squeeze(dim=1) # N, H, W

                # Prepare features for per-sample processing
                feature_before = feature_before.permute(0, 2, 3, 1).view(N, H*W, C).cpu().numpy()  # [N, H*W, C]
                feature_after = feature_after.permute(0, 2, 3, 1).view(N, H*W, C).cpu().numpy()    # [N, H*W, C]
                masks = foreground_mask.view(B, -1).cpu().numpy()
                
                foreground_codes = feature_index[foreground_mask.bool().expand_as(feature_index)]
                background_codes = feature_index[~foreground_mask.bool().expand_as(feature_index)]
                foreground_counts = torch.bincount(foreground_codes.flatten().int(), minlength=codebook.shape[0])
                background_counts = torch.bincount(background_codes.flatten().int(), minlength=codebook.shape[0])
                
                never_in_foreground = (foreground_counts == 0)
                foreground_dominant = (foreground_counts > background_counts)
                others = ~never_in_foreground & ~foreground_dominant
                index_mask = torch.zeros(codebook.shape[0], dtype=torch.float32)
                index_mask[foreground_dominant] = 1.0
                index_mask[others] = 0.5

                for method in ['pca', 'umap_15', 'umap_30', 'umap_100', 'tsne_5', 'tsne_30', 'tsne_100', 'umap_15_03', 'umap_30_03', 'umap_100_03', 'umap_15_05', 'umap_30_05', 'umap_100_05']:
                    # 生成codebook嵌入（保持不变）
                    if method == 'pca':
                        code_embedding = pca(codebook)
                    elif method == 'umap_15':
                        code_embedding = umap(codebook, n_neighbors=5)
                    elif method == 'umap_30':
                        code_embedding = umap(codebook, n_neighbors=15)
                    elif method == 'umap_100':
                        code_embedding = umap(codebook, n_neighbors=30)
                    elif method == 'umap_15_03':
                        code_embedding = umap(codebook, n_neighbors=5)
                    elif method == 'umap_30_03':
                        code_embedding = umap(codebook, n_neighbors=15)
                    elif method == 'umap_100_03':
                        code_embedding = umap(codebook, n_neighbors=30)
                    elif method == 'umap_15_05':
                        code_embedding = umap(codebook, n_neighbors=5)
                    elif method == 'umap_30_05':
                        code_embedding = umap(codebook, n_neighbors=15)
                    elif method == 'umap_100_05':
                        code_embedding = umap(codebook, n_neighbors=30)
                    elif method == 'tsne_5':
                        code_embedding = tsne(codebook, perplexity=5)
                    elif method == 'tsne_30':
                        code_embedding = tsne(codebook, perplexity=10)
                    elif method == 'tsne_100':
                        code_embedding = tsne(codebook, perplexity=30)

                    plt.figure(figsize=(18, 6*N))
                    for n in range(N):
                        # 对每个样本单独进行降维
                        before_features = feature_before[n]  # [H*W, C]
                        after_features = feature_after[n]    # [H*W, C]
                        
                        # 生成before/after嵌入
                        if method == 'pca':
                            emb_before = pca(before_features)
                            emb_after = pca(after_features)
                        elif method == 'umap_15':
                            emb_before = umap(before_features, n_neighbors=15, min_dist=0.1)
                            emb_after = umap(after_features, n_neighbors=15, min_dist=0.1)
                        elif method == 'umap_30':
                            emb_before = umap(before_features, n_neighbors=30, min_dist=0.1)
                            emb_after = umap(after_features, n_neighbors=30, min_dist=0.1)
                        elif method == 'umap_100':
                            emb_before = umap(before_features, n_neighbors=100, min_dist=0.1)
                            emb_after = umap(after_features, n_neighbors=100, min_dist=0.1)
                        elif method == 'umap_15_03':
                            emb_before = umap(before_features, n_neighbors=15, min_dist=0.3)
                            emb_after = umap(after_features, n_neighbors=15, min_dist=0.3)
                        elif method == 'umap_30_03':
                            emb_before = umap(before_features, n_neighbors=30, min_dist=0.3)
                            emb_after = umap(after_features, n_neighbors=30, min_dist=0.3)
                        elif method == 'umap_100_03':
                            emb_before = umap(before_features, n_neighbors=100, min_dist=0.3)
                            emb_after = umap(after_features, n_neighbors=100, min_dist=0.3)
                        elif method == 'umap_15_05':
                            emb_before = umap(before_features, n_neighbors=15, min_dist=0.5)
                            emb_after = umap(after_features, n_neighbors=15, min_dist=0.5)
                        elif method == 'umap_30_05':
                            emb_before = umap(before_features, n_neighbors=30, min_dist=0.5)
                            emb_after = umap(after_features, n_neighbors=30, min_dist=0.5)
                        elif method == 'umap_100_05':
                            emb_before = umap(before_features, n_neighbors=100, min_dist=0.5)
                            emb_after = umap(after_features, n_neighbors=100, min_dist=0.5)
                        elif method == 'tsne_5':
                            emb_before = tsne(before_features, perplexity=5)
                            emb_after = tsne(after_features, perplexity=5)
                        elif method == 'tsne_30':
                            emb_before = tsne(before_features, perplexity=30)
                            emb_after = tsne(after_features, perplexity=30)
                        elif method == 'tsne_100':
                            emb_before = tsne(before_features, perplexity=100)
                            emb_after = tsne(after_features, perplexity=100)

                        # Before特征子图
                        plt.subplot(N, 3, 3*n + 1)
                        plt.scatter(emb_before[masks[0]==0, 0], emb_before[masks[0]==0, 1], 
                                   s=5, c='gray', alpha=0.5, label='Background')
                        plt.scatter(emb_before[masks[0]==1, 0], emb_before[masks[0]==1, 1],
                                   s=20, c='red', alpha=0.8, marker='X', label='Foreground')
                        ...
                        plt.scatter(emb_after[masks[0]==0, 0], emb_after[masks[0]==0, 1],
                                   s=5, c='gray', alpha=0.5, label='Background')
                        plt.scatter(emb_after[masks[0]==1, 0], emb_after[masks[0]==1, 1],
                                   s=20, c='red', alpha=0.8, marker='X', label='Foreground')
                        ...
                        plt.scatter(code_embedding[index_mask==0, 0], code_embedding[index_mask==0, 1], 
                                   s=10, c='gray', alpha=0.5, label='Background')
                        plt.scatter(code_embedding[index_mask==0.5, 0], code_embedding[index_mask==0.5, 1],
                                   s=20, c='blue', alpha=0.8, marker='X', label='B & F')
                        plt.scatter(code_embedding[index_mask==1, 0], code_embedding[index_mask==1, 1],
                                   s=30, c='red', alpha=0.8, marker='X', label='Foreground')
