    ...
    for i, batch_data in pbar:
        ...
        with torch.no_grad():
            ...
            if True:#cav_content['agent_modality_list']==['m1', 'm2', 'm6']:
                ...
                feature_before = feature_before.permute(0, 2, 3, 1).view(N, H*W, C)
                feature_after = feature_after.permute(0, 2, 3, 1).view(N, H*W, C)
                fused_feature = fused_feature.permute(0, 2, 3, 1).view(B, H*W, C).cpu().numpy()

                combined_features = torch.cat([feature_before, feature_after], dim=0).cpu().numpy()
                ...
                for method in ['pca', 'umap_15', 'umap_30', 'umap_100', 'tsne_5', 'tsne_30', 'tsne_100']:
                    if method == 'pca':
                        combined_embedding = pca(combined_features.reshape(-1, C))
                        code_embedding = pca(codebook)
                        fused_embedding = pca(fused_feature.reshape(-1, C))
                    elif method == 'umap_15':
                        combined_embedding = umap(combined_features.reshape(-1, C), n_neighbors=15, min_dist=0.1)
                        code_embedding = umap(codebook, n_neighbors=5)
                        fused_embedding = umap(fused_feature.reshape(-1, C), n_neighbors=15, min_dist=0.1)
                    elif method == 'umap_30':
                        combined_embedding = umap(combined_features.reshape(-1, C), n_neighbors=30, min_dist=0.1)
                        ...
                        combined_embedding = umap(combined_features.reshape(-1, C), n_neighbors=100, min_dist=0.1)
                        ...
                        combined_embedding = umap(combined_features.reshape(-1, C), n_neighbors=15, min_dist=0.3)
                        code_embedding = umap(codebook, n_neighbors=5)
                        fused_embedding = umap(fused_feature.reshape(-1, C), n_neighbors=15, min_dist=0.3)
                    elif method == 'umap_30_03':
                        combined_embedding = umap(combined_features.reshape(-1, C), n_neighbors=30, min_dist=0.3)
                        code_embedding = umap(codebook, n_neighbors=15)
                        fused_embedding = umap(fused_feature.reshape(-1, C), n_neighbors=30, min_dist=0.3)
                    elif method == 'umap_100_03':
                        combined_embedding = umap(combined_features.reshape(-1, C), n_neighbors=100, min_dist=0.3)
                        code_embedding = umap(codebook, n_neighbors=30)
                        fused_embedding = umap(fused_feature.reshape(-1, C), n_neighbors=100, min_dist=0.3)
                    elif method == 'umap_15_05':
                        combined_embedding = umap(combined_features.reshape(-1, C), n_neighbors=15, min_dist=0.5)
                        code_embedding = umap(codebook, n_neighbors=5)
                        fused_embedding = umap(fused_feature.reshape(-1, C), n_neighbors=15, min_dist=0.5)
                    elif method == 'umap_30_05':
                        combined_embedding = umap(combined_features.reshape(-1, C), n_neighbors=30, min_dist=0.5)
                        code_embedding = umap(codebook, n_neighbors=15)
                        fused_embedding = umap(fused_feature.reshape(-1, C), n_neighbors=30, min_dist=0.5)
                    elif method == 'umap_100_05':
                        combined_embedding = umap(combined_features.reshape(-1, C), n_neighbors=100, min_dist=0.5)
                        code_embedding = umap(codebook, n_neighbors=30)
                        fused_embedding = umap(fused_feature.reshape(-1, C), n_neighbors=100, min_dist=0.5)
                    elif method == 'tsne_5':
                        combined_embedding = tsne(combined_features.reshape(-1, C), perplexity=5)
                        code_embedding = tsne(codebook, perplexity=5)
                        fused_embedding = tsne(fused_feature.reshape(-1, C), perplexity=5)
                    elif method == 'tsne_30':
                        combined_embedding = tsne(combined_features.reshape(-1, C), perplexity=30)
                        ...
                        combined_embedding = tsne(combined_features.reshape(-1, C), perplexity=100)
                        code_embedding = tsne(codebook, perplexity=30)
                        fused_embedding = tsne(fused_feature.reshape(-1, C), perplexity=100)

                    combined_embedding = combined_embedding.reshape(2*N, H*W, 2)
                    ...
                    for n in range(N):
                        ...
                        if n==0:
                            ...
                            plt.title(f'Codebook', fontsize=20)
                            plt.grid(alpha=0.2)
                            plt.legend(loc='upper right', fontsize=10)
                        elif n==1:
                            plt.scatter(fused_embedding[masks[0]==0, 0], fused_embedding[masks[0]==0, 1], s=5, c='gray', alpha=0.5, label='Background')
                            plt.scatter(fused_embedding[masks[0]==1, 0], fused_embedding[masks[0]==1, 1], s=20, c='red', alpha=0.8, marker='X', label='Foreground')
                            plt.title(f'Fused Feature', fontsize=20)
                            plt.grid(alpha=0.2)
                            plt.legend(loc='upper right', fontsize=10)

                    plt.tight_layout()
                    plt.savefig(save_path+f'/combined_distribution_{i}_{method}.jpg', dpi=600, bbox_inches='tight')
                    plt.close()
                    print(f"Saved to {save_path+f'/combined_distribution_{i}_{method}.jpg'}")
