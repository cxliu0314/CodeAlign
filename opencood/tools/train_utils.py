...
def check_missing_key(model_state_dict, ckpt_state_dict):
    ...
    print("Overwrite Keys from ckpt:")
    print(*set([keyname.split('.')[0] for keyname in checkpoint_keys]),sep='\n',end='\n\n')
...
def create_model(hypes, train_flag=True):
    """
    ...
    if model is None:
        print('backbone not found in models folder. Please make sure you '
              ...
              'called %s ignoring upper/lower case' % (model_filename,
                                                       target_model_name))
        exit(0)
    instance = model(backbone_config, train_flag)
    return instance
...
def setup_optimizer_codebook(hypes, model):
    method_dict = hypes['optimizer']
    optimizer_method = getattr(optim, method_dict['core_method'], None)
    if not optimizer_method:
        raise ValueError('{} is not supported'.format(method_dict['name']))
    
    # Separate codebook parameters
    codebook_params = []
    other_params = []
    ...
    optimizer_params = [
        {'params': other_params, 'lr': method_dict['lr']},
        {'params': codebook_params, 'lr': method_dict['lr'] * method_dict.get('codebook_lr_rate', 1)}
    ]
    
    if 'args' in method_dict:
        return optimizer_method(optimizer_params, **method_dict['args'])
    else:
        return optimizer_method(optimizer_params)
