...
def load_general_params_heter_group(param):
    """
    Based on the lidar range and resolution of voxel, calcuate the anchor box
    and target resolution.
    ...
    # add modality
    loader = yaml.Loader
    with open(param['heter']["definition_path"], 'r') as f:
        param_modal = yaml.load(f, Loader=loader)
    modality_list = param['heter']['ego_modality'] if isinstance(param['heter']['ego_modality'], list) else [param['heter']['ego_modality']]
    param['heter']['modality_setting'] = {}
    for m in modality_list:
        param['heter']['modality_setting'][m] = param_modal['heter']['modality_setting'][m]
        param['model']['args'][m] = param_modal['model']['args'][m]
