...
class HeterCodebookPyramidSharedHead(nn.Module):
    def __init__(self, args, train_flag=True):
        super(HeterCodebookPyramidSharedHead, self).__init__()
        self.args = args
    ...
    def forward(self, data_dict, save_path='', i=0, dis_path='/GPFS/data/changxingliu/HEAL/opencood/logs/test'):
        ...
        if save_path!='':
            ...
            for k in range(heter_feature_2d.shape[0]):
                avg_feature = heter_feature_2d[k].mean(dim=0)
                avg_feature = (avg_feature - avg_feature.min()) / (avg_feature.max() - avg_feature.min())
                img = avg_feature.mul(255).byte()
                save_image = torchvision.transforms.ToPILImage()(img)
                save_image.save(f"{save_path}/{i}_{k}_{agent_modality_list[k]}.jpg")
        ...
        if self.multi_channel_compressor_flag:
            ...
            heter_feature_2d_flat = heter_feature_2d.permute(0, 2, 3, 1).contiguous().view(-1, C)
            heter_feature_2d, code, _, codebook_loss = self.multi_channel_compressor(heter_feature_2d_flat)
            heter_feature_2d = heter_feature_2d.view(-1, H, W, C).permute(0, 3, 1, 2).contiguous()
            output_dict.update({'feature_before': heter_feature_2d_gt,
                                'feature_after': heter_feature_2d,
                                'feature_index': code,
            ...
            if save_path!='':
                for k in range(heter_feature_2d.shape[0]):
                    ...
                    img = avg_feature.mul(255).byte()
                    save_image = torchvision.transforms.ToPILImage()(img)
                    save_image.save(f"{save_path}/{i}_{k}_{agent_modality_list[k]}_code.jpg")
