...
#         out = out.dense()
#         B, C, D, H, W = out.shape
#         gt_bev_features = out.view(B, C * D, H, W)

#         return gt_bev_features
...
class BEVFusion(nn.Module):
    def __init__(self, args):
        ...
        self.lidar_encoder = PointPillar(args['encoder_args_lidar'])
        self.cam_encoder = LiftSplatShoot(args['encoder_args_cam'])
        self.fuser = ConvFuser(args['fuser'])
    def forward(self, data_dict, modality_name,multi_sensor):
        lidar_bev = self.lidar_encoder(data_dict,modality_name,multi_sensor=True)
        cam_bev = self.cam_encoder(data_dict,modality_name,multi_sensor=True)
        outs = self.fuser([lidar_bev,cam_bev])
        return outs
...
class ConvFuser(nn.Sequential):
    def __init__(self, args) -> None:
        ...
        super().__init__(
            nn.Conv2d(sum(self.in_channels), self.out_channels, 3, padding=1, bias=False),
