...
class HeterCodebookSharedHeadC2CInfer(nn.Module):
    def __init__(self, args, train_flag=True):
        super(HeterCodebookSharedHeadC2CInfer, self).__init__()
        self.args = args
        ...
        for modality_name in self.modality_name_list:
            ...
            Aligner building
            """
            if args.get("aligner", False) and modality_name in args.get("load_aligner", []):
                setattr(self, f"aligner_{modality_name}", AlignNet(args['aligner']))
                model_path = args['load_aligner'][modality_name]
                if model_path != '':
        ...
        if args.get("load_translator", False):
            for mod, model_path in args['load_translator'].items():
                setattr(self, f"translator_{mod}", Translator(args["translator"]))
                checkpoint = torch.load(model_path, map_location='cuda:{}'.format(local_rank))
                translator_state_dict = {k.replace(f'translator.', '', 1): v for k, v in checkpoint.items() if f'translator'==k.split('.')[0]}
                getattr(self, f"translator_{mod}").load_state_dict(translator_state_dict, strict=True)
                print(f"Translator for modality {mod} loaded successfully.")
    ...
    def forward(self, data_dict, vis_path='', data_balance=None):
        ...
        for i, modality_name in enumerate(self.modality_name_list):
            ...
            if self.args.get("aligner", False) and modality_name in self.args.get("load_aligner", []).keys():
        ...
        for modality_name in agent_modality_list:
            ...
            if modality_name in self.args['backend_modality']:
                # 直接通过codebook转换
                ...
                if self.use_d2d:
                    feature_2d = getattr(self, f"translator_{modality_name}")(spatial_aligned_feature, backend_modality).squeeze(0)
                else:
                    if self.use_coded_feature:
                        ...
                        feature_code2code = getattr(self, f"translator_{modality_name}")(coded_feature, backend_modality)
                        ...
                        feature_code2code = getattr(self, f"translator_{modality_name}")(one_hot.float(), backend_modality)
                    else:
                        # dense feature，translator转换，codebook decode
                        feature_code2code = getattr(self, f"translator_{modality_name}")(spatial_aligned_feature, backend_modality)
                    ...
                    if self.train_flag:
                        ...
                        quantizeds_code2code = torch.argmax(feature_code2code.permute(0, 2, 3, 1).contiguous().view(-1,1,C1), dim=2)
                        quantizeds_infer = torch.zeros(quantizeds_code2code.shape[0], 1, C1).to(quantizeds_code2code.device)
                        quantizeds_infer.scatter_(dim=2, index=quantizeds_code2code.unsqueeze(-1).long(), value=1)
                    ...
                    codemap_list.append(codemap)
                    feature_2d = self.backend_models[backend_modality]['codebook'].get_feature([quantizeds_infer])
                    feature_2d = feature_2d.view(-1, H, W, C).permute(0, 3, 1, 2).contiguous().squeeze(0)
                
                print(f"encoder {modality_name} backend {backend_modality}, with translator")
