...
class PyramidFusion_codebook(ResNetBEVBackbone):
    def __init__(self, model_cfg, input_channels=64):
        """
        Do not downsample in the first layer.
        """
    ...
    def forward_collab(self, 
        ...
        for i in range(self.num_levels):
            ...
            _, C, H, W = feature_list[i].shape
            B, L = affine_matrix.shape[:2]
            split_x = regroup(feature_list[i], record_len)
            # score = torch.sum(score, dim=1, keepdim=True)
            split_score = regroup(score, record_len)
            batch_node_features = split_x
            fused_feature_mlvl = []
            ...
            for b in range(B):
                ...
                fused_feature_mlvl.append(torch.sum(feature_in_ego * scores_in_ego, dim=0))
            fused_feature_mlvl = torch.stack(fused_feature_mlvl)
            fused_feature_list.append(fused_feature_mlvl)
        fused_feature = self.decode_multiscale_feature(fused_feature_list)
