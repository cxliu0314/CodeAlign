...
class HeteroFusionBlock(nn.Module):
    ...
    def warp_features(self, x, pairwise_t_matrix, mask, warpslow):
        # x: (B, L, C, H, W)
        ...
        for i in range(L):
            if warpslow:
                ...
                affine_matrix = normalize_pairwise_tfm(pairwise_t_matrix,
                                                        H, 
                                                        W, 
                                                        self.downsample_rate * self.discrete_ratio)
                x_agent = warp_affine_simple(x.reshape(B*L, -1, H, W),
                                                affine_matrix[:,i,:,:,:].reshape(-1, 2, 3),
                                                (H, W),
                                                align_corners=True).view(B, L, -1, H, W)
                one_mask = torch.ones((B, L, 1, H, W), device=x.device)
                com_mask = warp_affine_simple(one_mask.reshape(B*L, -1, H, W),
                                                affine_matrix[:,i,:,:,:].reshape(-1, 2, 3),
                                                (H, W),
                                                align_corners=True).view(B, L, -1, H, W).permute(0, 3, 4, 2, 1)
                com_mask = (com_mask > 0).float()
    ...
    def local_spatial_multi_agent_attention(self, x, pairwise_t_matrix, mask,
                                            mode, record_len, exclude_self, warpslow):
        # x: b l c h w
        # mask: b h w 1 l
        # window attention -> grid attention
        x_normed = self.window_norm(x.permute(0, 1, 3, 4, 2), mode).permute(0, 1, 4,
                                                                      2, 3)
        # print('x_nrom:',x_normed.shape)
        # print('pairwise_t_matrix:',pairwise_t_matrix.shape)
    ...
    def forward(self, x, pairwise_t_matrix, mode, record_len, mask, warpslow):
        # x: (B, L, C, H, W)
        # pairwise_t_matrix: (B, L, L, 4, 4)
        # mask: (B, H, W, 1, L, L)
