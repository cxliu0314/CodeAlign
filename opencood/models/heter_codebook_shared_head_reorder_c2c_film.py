...
from opencood.models.fuse_modules.FiLM_adapter import Adapter
import random


class HeterCodebookSharedHeadReorderC2CFiLM(nn.Module):
    def __init__(self, args, train_flag=True):
        super(HeterCodebookSharedHeadReorderC2CFiLM, self).__init__()
        self.args = args
        ...
        C2C Translator
        """
        self.translator = Adapter(args["translator"])
        ...
        if 'only_train_embedding' in args and args['only_train_embedding']:
            ...
            if self.train_flag:
                model_path = args['emb_path']
                if model_path != '':
                    local_rank = torch.distributed.get_rank()
                    checkpoint = torch.load(model_path, map_location='cuda:{}'.format(local_rank))
                    translator_model = getattr(self, 'translator')
                    translator_state_dict = {k.replace('translator.', ''): v for k, v in checkpoint.items() if 'translator' in k and not k.startswith('translator.adapter.emb_table')}
                    translator_model.load_state_dict(translator_state_dict, strict=True)
    ...
    def only_train_embedding(self):
        for p in self.parameters():
            p.requires_grad_(False)
        self.translator.adapter.emb_table.requires_grad_(True)
        # self.translator.adapter.film_generator.requires_grad_(True)
        # self.translator.adapter.film_generator.train()
