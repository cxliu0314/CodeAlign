...
class HeterModelLateCzc(nn.Module):
    def __init__(self, args,train_flag=None):
        super(HeterModelLateCzc, self).__init__()
        ...
        for modality_name in self.modality_name_list:
            ...
            setattr(self, f"fusion_backbone_{modality_name}", PyramidFusion(model_setting['fusion_backbone']))
            ...
            # self.fusion_backbone = PyramidFusion(model_setting['fusion']['backbone'])
            # self.shrink_flag = True
            # self.shrink_conv = DownsampleConv(model_setting['fusion']['shrink_head'])
    ...
    def forward(self, data_dict):
        ...
        fused_feature, occ_outputs = eval(f"self.fusion_backbone_{modality_name}").forward_single(feature)
        feature = eval(f"self.shrink_conv_{modality_name}")(fused_feature)
