...
class SimpleMLP(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.norm = nn.GroupNorm(1, args['dim'])
        layers = []
        for i in range(args['num_of_layers']):
            layers.append(nn.Conv2d(args['dim'], args['dim'], kernel_size=1, bias=True))
            if i < args['num_of_layers'] - 1:  # no activation after last layer
