...
class _multiCodebookQuantization(nn.Module):
    ...
    def argmaxSoftmax(self, logits: torch.Tensor, dim: int = -1):
        index = torch.argmax(logits, dim=dim, keepdim=True)
    ...
    def _sample(self, x: torch.Tensor, temperature: float):
        ...
        # sampled = self.argmaxSoftmax(logit)
...
class UMGMQuantizer(BaseQuantizer):
    ...
    def quantized(self, x: torch.Tensor):
        x_gt = x.detach()
        quantizeds = list()
        codes = list()
        for encoder in self._encoders:
            quantized, x, code, oneHot, logit = encoder(x_gt)
            quantizeds.append(quantized)
            codes.append(code)
        return quantizeds,codes
    
    def get_feature(self,quantizeds):
        formerLevel = None
        for decoder, quantized in zip(self._decoders[::-1], quantizeds[::-1]):
            formerLevel = decoder(quantized, formerLevel)
        return formerLevel
