...
def getIntermediateheterinferFusionDataset(cls):
    ...
    class Intermediate_heter_Infer_Fusion_Dataset(Intermediate_heter_Fusion_Dataset):
        ...
        def __init__(self, params, visualize, train=True):
            ...
            self.sensor_type_dict = { # for intermediate fusion
                'm1': 'lidar',
                'm2': 'lidar',
                'm3': 'lidar',
                'm4': 'lidar',
        ...
        def __getitem__(self, idx):
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                if sensor_type == "lidar":
                    input_lists[modality_name].append(selected_cav_processed[f"processed_features_{modality_name}"])
                elif sensor_type == "camera":
                    input_lists[modality_name].append(selected_cav_processed[f"image_inputs_{modality_name}"])
                else:
                    raise
            ...
            for modality_name in self.modality_name_list:
                if self.sensor_type_dict[modality_name] == "lidar":
                    merged_feature_dict = merge_features_to_dict(input_lists[modality_name]) 
                    ...
                    merged_image_inputs_dict = merge_features_to_dict(input_lists[modality_name], merge='stack')
