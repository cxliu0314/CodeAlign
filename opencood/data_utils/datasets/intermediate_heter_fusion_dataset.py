...
from opencood.utils.heter_utils import Adaptor, Adaptor_group
...
def getIntermediateheterFusionDataset(cls):
    ...
    class IntermediateheterFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            elif 'modality_assignment' in params:
                self.modality_assignment = params['modality_assignment']
            else:
            ...
            if self.modality_assignment is None:
                ...
                self.adaptor = Adaptor_group(self.ego_modality,
                                    self.modality_name_list,
                                    self.modality_assignment,
                                    lidar_channels_dict,
                                    mapping_dict,
                                    ...
                                    train,
                                    params)
                self.modality_assignment = self.adaptor.modality_assignment
        ...
        def __getitem__(self, idx):
            ...
            for modality_name in self.modality_name_list:
                input_lists[modality_name] = {}
                input_lists[modality_name]['input_list'] = []
            ...
            if self.visualize or self.kd_flag:
                ...
                for modality_name in self.ego_modality:
                    input_lists[modality_name]['input_list_proj'] = []
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                if sensor_type == "lidar":
                    input_lists[modality_name]['input_list'].append(selected_cav_processed[f"processed_features_{modality_name}"])
                    # eval(f"input_list_{modality_name}").append(selected_cav_processed[f"processed_features_{modality_name}"])
                elif sensor_type == "camera":
                    input_lists[modality_name]['input_list'].append(selected_cav_processed[f"image_inputs_{modality_name}"])
                ...
                if self.visualize or self.kd_flag:
                    ...
                    if sensor_type == "lidar" and self.kd_flag:
                        input_lists[modality_name]['input_list_proj'].append(selected_cav_processed[f"processed_features_{modality_name}_proj"])
            ...
            for modality_name in self.modality_name_list:
                if self.sensor_type_dict[modality_name] == "lidar":
                    merged_feature_dict = merge_features_to_dict(input_lists[modality_name]['input_list'])
                    ...
                    merged_image_inputs_dict = merge_features_to_dict(input_lists[modality_name]['input_list'], merge='stack')
            ...
            if self.kd_flag:
                ...
                for modality_name in self.modality_name_list:
                    processed_data_dict['ego'].update({
                        f'input_{modality_name}_proj': merge_features_to_dict(input_lists[modality_name]['input_list_proj']) # maybe None
        ...
        def collate_batch_train(self, batch):
            ...
            inputs_lists = {}
            for modality_name in self.modality_name_list:
                inputs_lists[modality_name] = {}
                inputs_lists[modality_name]['inputs_list'] = []
                inputs_lists[modality_name]['inputs_list_proj'] = []
            # inputs_list_m1 = [] 
            # inputs_list_m2 = []
            # inputs_list_m3 = []
            # inputs_list_m4 = []
            ...
            for i in range(len(batch)):
                ...
                for modality_name in self.modality_name_list:
                    if ego_dict[f'input_{modality_name}'] is not None:
                        inputs_lists[modality_name]['inputs_list'].append(ego_dict[f'input_{modality_name}'])
                ...
                if self.kd_flag:
                    ...
                    for modality_name in self.modality_name_list:
                        if ego_dict[f'input_{modality_name}_proj'] is not None:
                            inputs_lists[modality_name]['inputs_list_proj'].append(ego_dict[f"input_{modality_name}_proj"])
            ...
            # 2023.2.5
            for modality_name in self.modality_name_list:
                if len(inputs_lists[modality_name]['inputs_list']) != 0:
                    if self.sensor_type_dict[modality_name] == "lidar":
                        merged_feature_dict = merge_features_to_dict(inputs_lists[modality_name]['inputs_list'])
                        ...
                        merged_image_inputs_dict = merge_features_to_dict(inputs_lists[modality_name]['inputs_list'], merge='cat')
            ...
            if self.kd_flag:
                ...
                for modality_name in self.modality_name_list:
                    if len(inputs_lists[modality_name]['inputs_list_proj']) != 0 and self.sensor_type_dict[modality_name] == "lidar":
                        merged_feature_proj_dict = merge_features_to_dict(inputs_lists[modality_name]['inputs_list_proj'])
