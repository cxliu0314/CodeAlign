...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def generate_label(self, **kwargs):
        ...
        foreground_map = np.zeros((kwargs['anchors'].shape[0], kwargs['anchors'].shape[1]), dtype=np.uint8)
        offset_x = -self.params['cav_lidar_range'][0]
        offset_y = -self.params['cav_lidar_range'][1]
        ...
        for box in gt_standup_2d:
            ...
            y_start = int(np.floor((y_min + offset_y) / voxel_size))
            y_end = int(np.ceil((y_max + offset_y) / voxel_size))
            x_start, x_end = np.clip([x_start, x_end], 0, kwargs['anchors'].shape[0] - 1)
            y_start, y_end = np.clip([y_start, y_end], 0, kwargs['anchors'].shape[1] - 1)
            foreground_map[x_start:x_end + 1, y_start:y_end + 1] = 1

        # offset_x = -self.params['cav_lidar_range'][0]
        # offset_y = -self.params['cav_lidar_range'][1]
        # voxel_size = self.params['voxel_size'][0] * 2
        y_min, x_min, y_max, x_max = gt_standup_2d[:, 0], gt_standup_2d[:, 1], gt_standup_2d[:, 2], gt_standup_2d[:, 3]

        x_start = np.floor((x_min + offset_x) / voxel_size).astype(int)
        x_end = np.ceil((x_max + offset_x) / voxel_size).astype(int)
        y_start = np.floor((y_min + offset_y) / voxel_size).astype(int)
        y_end = np.ceil((y_max + offset_y) / voxel_size).astype(int)
        grid_shape = kwargs['anchors'].shape
        x_start = np.clip(x_start, 0, grid_shape[0] - 1)
        x_end = np.clip(x_end, 0, grid_shape[0] - 1)
        y_start = np.clip(y_start, 0, grid_shape[1] - 1)
        y_end = np.clip(y_end, 0, grid_shape[1] - 1)
        boxes_voxel = np.column_stack([x_start, y_start, x_end, y_end])

        label_dict = {'pos_equal_one': pos_equal_one,
                      'neg_equal_one': neg_equal_one,
                      'targets': targets,
                      'gt_boxes': boxes_voxel,
    ...
    def collate_batch(label_batch_list):
        ...
        for i in range(len(label_batch_list)):
            ...
            gt_boxes.append(label_batch_list[i]['gt_boxes']) # different lens
            foreground_map.append(label_batch_list[i]['foreground_map'])
